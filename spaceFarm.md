...
- [最初の検討  〜農業プラントの構想と問題点〜](#最初の検討--農業プラントの構想と問題点)
  ...
  - [内部に設置する栽培ユニットカートリッジ](#内部に設置する栽培ユニットカートリッジ)
  - [デブリ対策](#デブリ対策)
    - [デブリ速度は10km/s](#デブリ速度は10kms)
    - [しかし完全に防御しきるのは困難である。](#しかし完全に防御しきるのは困難である)
    - [バルーンプラントの貫通確率はおよそ1年で0.3％となる。](#バルーンプラントの貫通確率はおよそ1年で03となる)
    - [バルーンは守るのではなく補修・再使用・リサイクルで](#バルーンは守るのではなく補修再使用リサイクルで)
  ...
  - [宇宙酵母菌の可能性について](#宇宙酵母菌の可能性について)
    ...
    - [酵母で補うには？](#酵母で補うには)
    - [解決策](#解決策)
    - [良質な食糧を継続的に得るために](#良質な食糧を継続的に得るために)
    - [実際に調理してみた](#実際に調理してみた)
    - [味と今後の課題](#味と今後の課題-1)
...
- [宇宙で何を栽培するか](#宇宙で何を栽培するか)
  ...
  - [加工食材生産の必要性](#加工食材生産の必要性)
    - [大豆粉](#大豆粉)
    - [馬鈴薯でんぷん](#馬鈴薯でんぷん)
    - [糖類（果糖ブドウ糖液糖等）](#糖類果糖ブドウ糖液糖等)
    - [重曹](#重曹)
    - [バニラエッセンス](#バニラエッセンス)
    - [大豆油](#大豆油)
  - [改めて一日で必要な栄養素](#改めて一日で必要な栄養素)
  - [持ち込むスパイスと栽培可能性](#持ち込むスパイスと栽培可能性)
    - [バニラエッセンス](#バニラエッセンス-1)
    - [ガラムマサラ](#ガラムマサラ)
  - [食品の標準成分](#食品の標準成分)
  ...
  - [作物収量計算](#作物収量計算)
- [栽培ユニットカートリッジの改良](#栽培ユニットカートリッジの改良)
...
# 最初の検討  〜農業プラントの構想と問題点〜
ここより、農業プラントとは植物栽培専用の独立した比較的大型な区画空間を指すものとする。
試験管の中や手で引き出せる程度のボックスは含まない。
...
輸送時は小さく、展開時は充分な大きさの球形となり設備区画へ係留する。
...
戦車砲徹甲弾(APFSDS)は2km/sとされる。これよりデブリは高速なため貫通力も大きくなると考えるのが自然だろう。

### しかし完全に防御しきるのは困難である。

ISSきぼう実験棟のデータによると、デブリや微小隕石による10年間の非貫通確率PNPは0.9738以上とされる。
（船内実験室と船内保管室を合わせての値※1）

逆算して10年貫通確率は0.0262とする。


ISSきぼうモジュールのデブリ正面の断面積は実験室49.28m2 + 保管室18.48m2で67.76m2であり
直径10mのバルーンプラント断面積は78.5m2のため


貫通確率はISSきぼうモジュールと比較して1.158倍となる。


計算すると貫通確率は10年で0.0303

### バルーンプラントの貫通確率はおよそ1年で0.3％となる。
...
※1 "オンラインジャーナル2008年2月号/宇宙ステーション余話 第4回" 日本プロジェクトマネジメント協会 (2008年2月)

### バルーンは守るのではなく補修・再使用・リサイクルで

強固な外殻よりも補修・再生しやすいバルーンタイプを採用する。
何故ならば地球地表の温室もひょうの多い地域ではガラス製よりビニールハウスの方が再生容易のためである。

内部の栽培ユニットカートリッジも再生して使用する。



### 循環表層水防護膜「武者鎧」

前項で述べた通り居住区画ではないため大規模な隔壁は設けない

年間1％程度の損失で済む程度にできるようスタッフィング入りバンパネットを16分割にてバルーンプラントに覆う形で設置する。

更にネットとバルーンプラントの間に水タンクを兼ねた循環式水バルーン層「武者鎧」を31400分割にて配置し防護性能を強化する。

バルーンプラント自体の想定重量は700kg 
破壊されたバルーンプラントは内部含めリサイクル処理に回す。
ネットと水バルーンは破損部を交換補修し再使用する。
...
## 宇宙酵母菌の可能性について
### 宇宙生活で不足するタンパク質
成人男性でタンパク質1日60ｇ必要※１　1食当たり20ｇ

エネルギーは１日2650kcal、1食当たり884kcal

前項［カロリーとコスト考察］のレシピでは蛋白質９ｇ、エネルギー638kcal不足する。

| |重量g|カロリーkcal|タンパク質g|コスト￥|
|:----|:----|:----|:----|----:|
|1日分必要量||2650|60||
|1食分必要量||884|20||
|以前のレシピ1食分|213|246|11|1030000|
|不足||638|9||
...
### 問題点
宇宙で畜産を行うのは飼料効率が悪いため除外（前項［農業プラントの必要性］より）
植物性タンパク質だけでは例えばエダマメに頼る場合大豆エストロゲン過剰摂取による弊害がある。これについては［宇宙で何を栽培するか］にて後述する。
動物性タンパク質生産には魚類養殖もあるが、プラントが比較的大型になるため初期は難しい。
...
| |含有量/100g|必要タンパク質g|必要な酵母の量g|大さじ|
|:----|:----|:----|:----|----:|
|EBIOSビール酵母|55|9|16|1と1/8|
|パン用ドライイースト|45|9|20|1と1/3|
...
## ジャガイモ
炭水化物摂取源として穀物を検討したが、水耕栽培の実績があるものはジャガイモしかなく、それもキリンのエアポニックス研究によるものであったため必然的にジャガイモのみとなった。

## ダイズ（エダマメ）

タンパク質摂取源としてもカロリー補給源としても豆類が優秀であったため、水耕栽培実績があり汎用性のあるダイズを選択。成熟度の変化によりエダマメとしても使用するものとした。

## 葉物野菜類
ミネラル等摂取に重要である葉物野菜は水耕栽培でもはや定番のレタスと、炒め物に向き早生種のあるチンゲンサイ、香味野菜のパクチーを選択した。

## 結実野菜類
ビタミン等摂取に重要である結実野菜は水耕栽培定番であり貴重な甘みがあるトマトとイチゴ、JAXAの資料にて選定されていたキュウリを選択した。

## 菌茸類
食感他の補填やうま味を生成する作物として菌茸を選択。
食感も踏まえた上で菌床栽培実績のあるシイタケ、ブナシメジ、アラゲキクラゲを選択した。


## 一人当たりの必要栄養素

1日喫食上限量を3000gと仮定して
カロリーとタンパク質を食糧より摂取し、その他の栄養素に関しては地上より持ち込むサプリメント等により補給するとする。

なお、スパイス類も地上より持ち込むものとしてここからは取り扱う。

これに関しては1kg/100万円の打ち上げコストがかかると仮定した場合、1gであれば1000円となる。

これであれば各々現地製造するより地上から打ち上げたほうがコストが安く済むからである。

未加工で栄養成分を摂れタンパク質60gとカロリー2650kcalが１日の摂取量に足るように計算した表がこちら

| |一人当たりの必要重量/day|カロリーkcal/100g|タンパク質g/100g|一人当たりのカロリー数/day|一人当たりのタンパク質/day|
|:----|:----|:----|:----|:----|:----|
|ジャガイモ|100|59|1.8|59|1.8|
|ダイズ(エダマメ)|1896|125|11.7|2370|221.832|
|パンケーキミックス|0|70|1|0|0|
|大豆油|0|885|0|0|0|
| | | | | | |
|レタス|150|13|0.8|19.5|1.2|
|チンゲンサイ|50|9|0.6|4.5|0.3|
|パクチー|50|18|1.4|9|0.7|
| | | | | | |
|イチゴ|300|31|0.9|93|2.7|
|トマト|100|20|0.7|20|0.7|
|キュウリ|100|13|1|13|1|
| | | | | | |
|シイタケ|100|25|3.1|25|3.1|
|ブナシメジ|100|26|2.7|26|2.7|
|アラゲキクラゲ|50|14|0.7|7|0.35|
| | | | | | |
|合計|2996|1308|26.4|2646|236.382|

この表は未加工で考えたが、この場合1日3000g以内に抑えたとしても喫食しにくい形状である。
大量のふかし芋の上に大量の豆とキノコが乗っかっている状態を想像すれば理解いただけるだろうか。
そのため食材を適宜油等に加工する必要が生じる。

## 加工食材生産の必要性
ここで栽培された食材を生で使うのではなく工業的に加工して使う必要が生じた。
必要な加工食材は大きく分けて
- パンケーキ（ホットケーキ）ミックス/HM
- 大豆油

更にパンケーキミックスの材料を細分化すると
- 大豆粉
- 馬鈴薯でんぷん
- 糖類
- 重曹
...
### 大豆粉
大豆を粉末にする。宇宙で可能だと考えるが無重力対応電動ミルが必要。

### 馬鈴薯でんぷん
馬鈴薯（ジャガイモ）より生成。宇宙で可能だと考えるが製粉機が必要。

### 糖類（果糖ブドウ糖液糖等）
馬鈴薯でんぷんを酵素の効果で生成。宇宙で可能だと考えるがドレーン/抽出装置が必要。

### 重曹
宇宙長期滞在の前提として尿を水と塩（塩化ナトリウム）に分離する際炭酸ナトリウムを生成するのでそこから炭酸水素ナトリウムにて生成。

宇宙で可能だと考える。

### バニラエッセンス
バニラの苗を育成してバニラビーンズを生成した後熟成に230日ほど、その後キュアリングで21日発酵・熟成させる必要があるため非常に手間と時間がかかり水耕栽培の成功例もない。更にエッセンスに加工する必要もある。

宇宙で不可能ではないが現実的ではなく、使用量も僅かなのでスパイス類同様地上から持ち込むことになろう。

### 大豆油
大豆を圧搾して生成。宇宙で可能だと考えるが圧搾機が必要。搾りかすはおからや大豆ミートとして活用。
...
※Jaxaリポジトリ　月面農場ワーキンググループ検討報告書　第１版　3.作物種の選定 表3.1 男性の項目より

## 持ち込むスパイスと栽培可能性
### バニラエッセンス
前項で述べた通り、加工に多大な手間暇を要する為地上より持ち込むこととする

### ガラムマサラ
カレースパイスとして調合したもの。クミンやコリアンダー等栽培できそうなものもあるがウコンのような根菜は難易度が高いと推定される。

また一回の使用量も数gのため地上から持ち込むものとする。

## 食品の標準成分
ここに使用する加工食品の標準的な栄養成分を転載する。

|大豆加工品|カロリー|タンパク質|食物繊維|廃棄率/％|
|:----|:----|:----|:----|:----|
|大豆油|885|0|0|78|
|大豆ミート| | | | |
|おから|363|6.1|11.5| |
...
※廃棄率は農水省第14号特別分析トピック　我が国と世界の油脂をめぐる動向より、それ以外は文科省日本食品標準成分表（八訂）増補2023年第2章より転載。
...
弊害として大豆エストロゲン過剰摂取による女性ホルモンと類似するエストロゲンレセプター結合が引き起こす種々の生体作用も問題視されている。※1

しかしエダマメはイソフラボン含有量が10mg/100g※2であるため一日の摂取許容量が1250gとなる※1ため問題となりにくい。

これに対してエダマメが成熟した大豆はイソフラボン含有量が平均247mg/100g※1であるため一日の摂取許容量が50gとなりそのため問題になりうる。

このため健康被害に対する予防線を張り許容量を超えない手段として前項［農業プラントの必要性］より畜産ではなく酵母菌の活用を検討する。

※1　農林水産省　大豆及び大豆イソフラボンに関するQ＆A
※2　独立行政法人国立健康・栄養研究所　FAQ 枝豆にはイソフラボンは含まれますか？

## 酵母菌を培養してタンパク質の植物性偏りを補正する
前項の［宇宙酵母菌の可能性］で解説したとおり、植物性イソフラボン過剰摂取を予防するため培養槽で生産できる酵母菌使用を想定する。
...
※宇宙航空研究開発機構特別資料　JAXA-SP-19-001 30p 表3.5よりジャガイモ・ダイズ・レタス・イチゴ・トマト・キュウリの数字を引用


# 栽培ユニットカートリッジの改良
前項［内部に設置する栽培ユニットカートリッジ］にて想定した栽培部分には作物による差異を考慮していなかった。
ここでは重要な作物であるエダマメに適した栽培ユニットカートリッジを考察したい。

## 総統との計算千本ノック
まず、宇宙空間にて実現可能な強度と輸送重量を兼ね備えた案を考察するため総統と共に議論と計算を重ねた。
...
この栽培ユニットの光源は当初、下図のように栽培バルーンプラントの中心部に配置する予定であった。
...
発熱を充分冷却するために光源を栽培ユニットカートリッジに付属する形で設置してみることにした。
...
栽培バルーンプラントの想定構成上、栽培ユニットカートリッジ一つ当たり4.6kg程度に抑える必要があった。
...
栽培ユニットモジュールを上下反転して武者鎧の水循環にてLEDユニットの冷却を試みた。更に栽培ユニットカートリッジは気密性を考慮していないため冷却と空気循環の為下部にファンユニットを付け上部から排気するようにした。

<img alt="picture 51" src="images/6ec4517537d205b72a7f03232d57112864d427aca2222629063ff07d6458a663.png" width="400" />  

この案では栽培ユニットカートリッジを設置する栽培プラントバルーン内を１気圧とした。

しかし栽培プラントバルーンの強度が不足するため、まずバルーン内圧力を再検討する。

<img alt="picture 116" src="images/94909fa0584799721c9b2f3df316353fafad090c4e01b746723c4aacff715229.png" width="400" />  

プラントのバルーン部は内圧を当初１気圧で考えていたが、素材の問題により0.1気圧とする。

和紙＋こんにゃくというのはWW2の太平洋戦争にて旧日本帝国軍が開発した風船爆弾の風船部分である。

このように栽培プラントバルーンは0.1気圧にて高強度の非金属を使用する事を前提とした結果、栽培ユニットカートリッジを気密化し内圧差に耐える必要があり、強度不足の懸念があったため次案を考えた。
...
前項にて栽培プラントバルーン内部の気圧を材質の都合上低下（0.1気圧）

そのため栽培ユニットカートリッジ内部（0.5気圧）との内圧差が生じるようになった。
それを考慮して栽培ユニットカートリッジを圧力に強いシリンダー型とした。シリンダーの直径を25cmと小さくする。
...
シリンダー内圧が0.5気圧、外部（プラント内）が0.1気圧の場合強い力が外部に向けかかる。
...
内圧に耐えるため、栽培ユニットカートリッジの蓋を排しバルーン内部にねじ切りをした基部を設置することで解決しようとした。所謂ペットボトルを逆さにしたような構造となる。
...
当初は栽培ユニットカートリッジを硬質素材による再利用を考えた構造を想定していたが思い切って使い捨て前提の構造とし、メンテナンスや収穫の度にロール状のパウチを引き出して上部を加熱圧着することで圧力に耐える構造とする。


ここまでくると栽培ユニットカートリッジとは言えないため今後は栽培ユニットパウチと呼称する。
...
ようやく栽培ユニットパウチ一つを4.6kg以内に抑えることができた。
...
エダマメ一つの栽培ユニットを最適化するだけでこれだけ仕様が変わりもはやカートリッジとは言えず（表記が栽培ユニットパウチと変わったのはその為である）栽培プラント自体の構造にも影響を与えた。
